...
//        self.img.image = UIImage.animatedImageWithAnimatedGIFData(NSData.init(contentsOfURL: url!)!)
        ...
        self.btnFacebook.addTarget(self, action: #selector(self.facebookBtnClicked), for: UIControlEvents.touchUpInside)
            ...
            case .success(let grantedPermissions, let declinedPermissions, let accessToken):
                ...
                connection.add(GraphRequest(graphPath: "/me" , parameters: ["fields": "name,email,first_name,last_name,picture.type(small),gender"])) { httpResponse, result in
                    ...
                    case .success(let response):
                        ...
                        self.facebookgender = response.dictionaryValue!["gender"] as! String
                        self.facebookFirstName = response.dictionaryValue!["first_name"] as! String
                        self.facebookLastName = response.dictionaryValue!["last_name"] as! String
                        self.facebookMail = response.dictionaryValue!["email"] as! String
//                        self.facebookURL = response.dictionaryValue!["last_name"] as! String
                        self.faceBookID = response.dictionaryValue!["id"] as! String

                        
                        if Token == ""
                        {
                            self.getTokenBeforeFacebookLogin()
                        }
                        else
                        {
...
//        MyJSONParser.objectWithUrlString("\(URL)?case=lg&usr=gketa_77@yahoo.com&pwd=open&si=\(Token)", responseDelegate: self, responseSelector: #selector(LoginViewController.LoginCallbk(_:)))
    ...
    //MARK:- service login with FB
    func getTokenBeforeFacebookLogin()
    {
        showLoading()
        MyJSONParser.object(withUrlString: "\(URL)?case=hs&p1=mazinger5596&p2=CaptenMaged5_0&p3=Mazinger_1169&p4=Denam0_9987&p5=Dokfleed_3320", responseDelegate: self, responseSelector: #selector(LandingViewController.TokenBeforeFacebookLoginCallBk(_:)))
    }
    
    func TokenBeforeFacebookLoginCallBk(_ dict : [String : AnyObject])
    {
        if let errorStatus = dict["Msg"] as? String
        {
            if errorStatus == "0"
    ...
    {
        showLoading()
        MyJSONParser.object(withUrlString: "\(URL)?case=fblr&em=\(self.facebookMail)&id=\(self.faceBookID)&lng=\(languageID)&fn=&si=\(Token)", responseDelegate: self, responseSelector: #selector(LandingViewController.callFacebookLoginCallBk(_:)))
    }
    
    func callFacebookLoginCallBk(_ dict : [String : AnyObject])
    {
        print("ok")
        hideLoading()
        if let errorStatus = dict["Msg"] as? String
